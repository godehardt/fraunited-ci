...
class Match
    ...
    def make_team_start_script side, team
        ...
        File.open(script, "w") do |file|
            ...
            Range.parse(@config.agent_range).each do |num|
                if get_host(side, num) == "localhost"
                    file.puts "#{team}/start #{@config.server} #{team} #{num} #{@config.team_mode} & >> #{@log_dir}/team_#{side}-output.log 2>> #{@log_dir}/team_#{side}-error.log"
                else
    ...
    def stop_teams
        ...
        hosts_left.each do |host|
            if host == "localhost"
                system "'#{@team_l}/kill' >> #{@log_dir}/team_l-output.log 2>> #{@log_dir}/team_l-error.log"
            else
        ...
        hosts_right.each do |host|
            if host == "localhost"
                system "'#{@team_r}/kill' >> #{@log_dir}/team_r-output.log 2>> #{@log_dir}/team_r-error.log"
            else
    ...
    def save_logging side, team
        if FileTest.exists? File.join(team, "save_logging")
            ...
            (side == "l" ? hosts_left : hosts_right).each do |host|
                Range.parse(@config.agent_range).each do |num|
                    if host == "localhost"
                        system "'#{team}/save_logging #{@config.server} #{team} #{num} #{host} #{File.join(Dir.pwd, @log_dir)} #{@config.team_mode}' >> #{@log_dir}/team_#{side}-output.log 2>> #{@log_dir}/team_#{side}-error.log"
                    else
